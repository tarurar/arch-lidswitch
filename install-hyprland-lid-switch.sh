...
snapshot_installation_state() {
    ...
    main_was_active=$(capture_active_state lid-monitor.service)
    resume_was_active=$(capture_active_state lid-resume-monitor.service)
    {
...
activate_installed_services() {
    ...
    if ! systemctl --user is-active --quiet lid-monitor.service; then
        ...
        systemctl --user status lid-monitor.service || true
        return 1
    fi
    if ! systemctl --user is-active --quiet lid-resume-monitor.service; then
        log_error "Failed to start lid resume monitor service"
        systemctl --user status lid-resume-monitor.service || true
        return 1
    fi
    log_success "Lid monitor and resume services are running"
    ...
    if [[ "$service_recovery_required" == true ]]; then
        ...
        if (( service_restore_status == 0 )); then
            if [[ "$main_was_active" == true ]]; then
                systemctl --user start lid-monitor.service >/dev/null 2>&1 \
                    || service_restore_status=1
            else
                systemctl --user stop lid-monitor.service >/dev/null 2>&1 || true
            fi
            if [[ "$resume_was_active" == true ]]; then
                systemctl --user start lid-resume-monitor.service >/dev/null 2>&1 \
                    || service_restore_status=1
            else
                systemctl --user stop lid-resume-monitor.service >/dev/null 2>&1 || true
            fi
            if [[ "$(capture_active_state lid-monitor.service)" != \
                "$main_was_active" || \
                "$(capture_active_state lid-resume-monitor.service)" != \
                "$resume_was_active" ]]; then
                log_error "Could not restore prior service active state"
                service_restore_status=1
            fi
        fi
        if (( service_restore_status != 0 )); then
