...
refresh_layer_client_unit() {
    ...
    # Hyprland can finish adding, moving, and removing layer surfaces shortly
    # after its monitor topology already reports the requested state. Let that
    # transition settle before replacing the layer client and its output-local
    # surfaces.
    sleep 0.2
    if timeout --kill-after=1s 2s \
